...
import {
  ...
  InviteMemberModal,
  ConfirmModal,
} from "./components/modals/AdminVMModals";
...
import { AddonServiceProvider } from "./store/addonServiceStore";
...
import { SystemSettingsProvider } from "./store/systemSettingsStore";
...
const AppInner = ({
  ...
  tw: TweakState;
  setTweak: (
    keyOrEdits: keyof TweakState | Partial<TweakState>,
    value?: any,
  ) => void;
}) => {
  ...
  const [prefillCustomerId, setPrefillCustomerId] = useState<
    string | undefined
  >(undefined);
  const [prefillRequestId, setPrefillRequestId] = useState<string | undefined>(
    undefined,
  );
  const [prefillRequestType, setPrefillRequestType] = useState<
    "vm" | "addon" | undefined
  >(undefined);
  ...
  const openModal = (kind: string, props: any = {}) => {
    setModalKind(kind);
    setModalProps(props);
  };
  const closeModal = () => {
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === "n") {
        nPressed = true;
        gPressed = false;
        setTimeout(() => (nPressed = false), 1200);
        return;
      }
      if (gPressed) {
  ...
  return (
    <div className="app">
      {tw.role === "Customer" ? (
        <CustomerPortal
          key="customer-portal"
          setRole={(r: string) => setTweak("role", r)}
          roleNames={tw.roleNames || {}}
        />
      ) : (
        <>
          <Sidebar
            ...
            role={tw.role}
            roleNames={tw.roleNames || {}}
            onAccountClick={() => handleSetView("account")}
          ...
          <div className="main">
            ...
            {view === "tasks" && (
              <TasksView
                openTask={openTask}
                setView={handleSetView}
                setAutoOpenQuote={setAutoOpenQuote}
            ...
            {drawerTaskId && (
              <TaskDrawer
                requestId={drawerTaskId}
                onClose={closeTaskDrawer}
                userRole={tw.role}
              />
            )}
            {view === "network" && (
              <NetworkView openVM={openVM} openModal={openModal} />
            )}
            {view === "console" && (
              <PlaceholderView
                title="Web Console"
                description="Proxmox web console - coming soon"
              />
            )}
            {view === "nodes" && (
              <PlaceholderView
                title="Proxmox Nodes"
                description="Node management view - coming soon"
              />
            )}
            {view === "topology" && (
              <PlaceholderView
                title="Network Topology"
                description="Network topology view - coming soon"
              />
            )}
            {view === "snapshots" && (
              <PlaceholderView
                title="Snapshots"
                description="VM snapshots view - coming soon"
              />
            )}
            {view === "maintenance" && (
              <PlaceholderView
                title="Maintenance Windows"
                description="Maintenance scheduling - coming soon"
              />
            )}
            {view === "patches" && (
              <PlaceholderView
                title="Patch Queue"
                description="OS patch management - coming soon"
              />
            )}
            {view === "firewall" && (
              <PlaceholderView
                title="Firewall Rules"
                description="Firewall configuration - coming soon"
              />
            )}
            ...
            {drawerCustId && (
              <CustomerDrawer
                custId={drawerCustId}
                onClose={closeCustDrawer}
                openVM={openVM}
              />
            )}
            {view === "customer-accounts" && (
              <CustomerAccountManagementView
                openCust={openCust}
                openModal={openModal}
                setView={handleSetView}
                role={tw.role}
            ...
            {view === "pipeline" && (
              <PlaceholderView
                ...
                description="Sales pipeline view - coming soon"
              />
            )}
            {view === "quotes" && (
              <QuotesView
                autoOpen={autoOpenQuote}
                onAutoOpenReset={() => {
                  setAutoOpenQuote(false);
                  setPrefillCustomerId(undefined);
                  setPrefillRequestId(undefined);
                  setPrefillRequestType(undefined);
                }}
            ...
            {view === "followups" && (
              <PlaceholderView
                title="Follow-ups"
                description="Sales follow-ups - coming soon"
              />
            )}
            {view === "trials" && (
              <PlaceholderView
                title="Trial Conversions"
                description="Trial conversion tracking - coming soon"
              />
            )}
            ...
            {view === "account" && (
              <AccountSettingsView role={tw.role} setView={handleSetView} />
            )}
          </div>
          ...
          {modalKind === "renew" && modalProps.vm && (
            <RenewModal vm={modalProps.vm} onClose={closeModal} />
          )}
          {modalKind === "spec" && modalProps.vm && (
            <SpecModal vm={modalProps.vm} onClose={closeModal} />
          )}
          {modalKind === "terminate" && modalProps.vm && (
            <TerminateModal vm={modalProps.vm} onClose={closeModal} />
          )}
          {modalKind === "delete" && modalProps.vm && (
            <DeleteModal vm={modalProps.vm} onClose={closeModal} />
          )}
          {modalKind === "newtask" && (
            <NewTaskModal
              onClose={closeModal}
              presetStatus={modalProps.status}
            />
          )}
          {modalKind === "newcust" && (
            <NewCustomerModal
              onClose={closeModal}
              onPasswordGenerated={(email, tempPassword, name) => {
                setTempPasswordData({ email, tempPassword, name });
                setModalKind(null);
              }}
            />
          )}
          ...
          {modalKind === "newinvoice" && (
            <NewInvoiceModal
              onClose={closeModal}
              presetCustomer={modalProps.customer}
            />
          )}
                                  ...
                                  <Router>
                                    <Routes>
                                      ...
                                      <Route
                                        ...
                                        element={<TeamChangePasswordPage />}
                                      />
                                      <Route
