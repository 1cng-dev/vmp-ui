...
const AdminDirectVMCreate: React.FC = () => {
  ...
  const confirmSubmit = async () => {
    ...
    try {
      ...
      if (!f.customer) {
        toast("Customer selection is required", "bad");
        setIsSubmitting(false);
        return;
      }
      // Legacy ID is now required
      if (!f.legacy_id) {
      ...
      vmId = await addVM({
        ...
        task_type: "new",
        legacy_id: f.legacy_id, // Now required
        ...
        <div className="card">
          ...
          <div className="card-body">
            <div className="grid-2" style={{ gap: 12 }}>
              <div className="field">
                <label>
                  Legacy ID <span style={{ color: "var(--bad)" }}>*</span>
                </label>
                <input
                  value={f.legacy_id}
                  onChange={(e) => set("legacy_id", e.target.value)}
                  placeholder="e.g. VPS-TDC-0001-3000-1CNG"
                />
              </div>
              <div className="field">
                <label>
