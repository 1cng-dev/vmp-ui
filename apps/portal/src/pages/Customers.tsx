...
import { StatusPill, Avatar, formatMMK } from '../components/ui/ui'
...
const CustomersView: React.FC<CustomersViewProps> = ({ openCust, openModal }) => {
  ...
  const filters = [
    ...
    { id: 'Pending', label: 'KYC pending', count: customers.filter(c => c.kyc === 'Pending').length },
    { id: 'Rejected', label: 'KYC rejected', count: customers.filter(c => c.kyc === 'Rejected').length },
    ...
    <div className="content">
      <div className="page-head">
        <div>
          ...
          <p className="page-subtitle">{customers.length} accounts · {customers.filter(c => c.kyc === 'Pending').length} pending KYC</p>
        </div>
