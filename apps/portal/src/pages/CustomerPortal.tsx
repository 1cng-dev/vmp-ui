...
export const CustomerPortal: React.FC<CustomerPortalProps> = ({ setRole: _setRole, roleNames: _roleNames = {} }) => {
  ...
  const waitingForCustomerProfile = !!auth?.user && !me && customers.length === 0
  ...
  // Wait for customer data to be loaded before rendering to prevent incorrect KYC status display
  if (!auth?.user || customersLoading || waitingForCustomerProfile || (!me && customers.length > 0)) {
    return (
