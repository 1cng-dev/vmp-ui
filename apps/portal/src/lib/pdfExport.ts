...
export const generateInvoicePDFBase64 = async (invoice: any, customer: any): Promise<string> => {
...
export const exportInvoiceToPDFAuto = async (invoice: any, _customer: any) => {
  ...
  const link = document.createElement('a')
  link.href = url
  link.download = `Invoice-${invoice.legacy_id || invoice.id}.pdf`
  link.target = '_self'
  link.rel = 'noopener noreferrer'
  link.style.display = 'none'
  document.body.appendChild(link)
