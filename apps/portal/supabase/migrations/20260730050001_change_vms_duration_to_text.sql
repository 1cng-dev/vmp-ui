-- Change duration field from INTEGER to TEXT in vms table
...
-- Allow formats: "14 days", "1 month", "3 months", "1 month 14 days", etc.
ALTER TABLE public.vms 
ADD CONSTRAINT vms_duration_format 
CHECK (duration ~ '^(\d+ (day|days|month|months)( \d+ (day|days|month|months))?)$');
